...
class MSRResNet_Head_noiser1(nn.Module):
    ''' modified SRResNet'''

    def __init__(self, in_c=3, out_c=3, nf=64, scale=1, require_modulation=False, degradation_embed_dim=512):
        super(MSRResNet_Head_noiser1, self).__init__()
    ...
    def forward(self, x):
        out = self.lrelu(self.norm(self.conv_first(x)))
...
class MSRResNet_Head_noiser2(nn.Module):
    ''' modified SRResNet'''

    def __init__(self, in_c=3, out_c=3, nf=64, scale=1, require_modulation=False, degradation_embed_dim=512):
        super(MSRResNet_Head_noiser2, self).__init__()
    ...
    def forward(self, x):
        ...
        out = self.basic_block3(out)
        out = self.basic_block4(out)
        out = self.norm(self.HRconv(out))
        out = self.conv_last(self.lrelu(out))
        
        return out

class MSRResNet_Head_noiser1_2(nn.Module):
    ''' modified SRResNet'''

    def __init__(self, in_c=3, out_c=3, nf=64, scale=1, require_modulation=False, degradation_embed_dim=512):
        super(MSRResNet_Head_noiser1_2, self).__init__()
    ...
    def forward(self, x):
        out = self.lrelu(self.norm1(self.conv_first(x)))

        out = self.basic_block1(out)
        out = self.basic_block2(out)
        out = self.basic_block3(out)
        out = self.basic_block4(out)
        out = self.norm2(self.HRconv(out))
        out = self.conv_last(self.lrelu(out))
