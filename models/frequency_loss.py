...
class FrequencyLoss4(nn.Module):
    ...
    def __init__(self,
                 ...
                 log_matrix=False,
                 batch_matrix=False):
        super(FrequencyLoss4, self).__init__()
    ...
    def tensor2freq(self, x):
        ...
        magnitude = torch.clamp(magnitude, min=0., max=1.)
        freq = torch.stack([y, magnitude], -1)
        return freq

    def loss_formulation(self, recon_freq, real_freq, matrix=None):
     # frequency distance using (squared) Euclidean distance
        tmp = (recon_freq - real_freq) ** 2
        #loss = torch.sqrt(tmp[..., 0] * 0.8 + tmp[..., 1] + 1e-12) ** self.loss_gamma

        loss1 = torch.sqrt(tmp[..., 0] * 0.8 + 1e-12)
        loss2 = torch.sqrt(tmp[..., 1] + 1e-12)
        if self.with_matrix:
...
class FrequencyLoss3(nn.Module):
    def __init__(self,
                 loss_gamma=1.,
                 matrix_gamma=1.,
                 patch_factor=1,
                 ...
                 log_matrix=False,
                 batch_matrix=False):
        super(FrequencyLoss3, self).__init__()
        self.loss_gamma = loss_gamma
        self.matrix_gamma = matrix_gamma
        self.patch_factor = patch_factor
        self.ave_spectrum = ave_spectrum
    ...
    def tensor2freq(self, x):
        ...
        freq = torch.stack([magnitude], -1)
        return freq

    def loss_formulation(self, recon_freq, real_freq, matrix=None):
        # frequency distance using (squared) Euclidean distance
        tmp = (recon_freq - real_freq) ** 2
        loss = torch.sqrt(tmp[..., 0] + 1e-12) ** self.loss_gamma
        if self.with_matrix:
            # spectrum weight matrix
            if matrix is not None:
                # if the matrix is predefined
...
class FrequencyLoss2(nn.Module):
    ...
    def __init__(self,
                 ...
                 log_matrix=False,
                 batch_matrix=False):
        super(FrequencyLoss2, self).__init__()
        self.loss_gamma = loss_gamma
        self.matrix_gamma = matrix_gamma
        self.patch_factor = patch_factor
        self.ave_spectrum = ave_spectrum
...
class FrequencyLoss1(nn.Module):
    ...
    def __init__(self,
                 ...
                 log_matrix=False,
                 batch_matrix=False):
        super(FrequencyLoss1, self).__init__()
        self.loss_gamma = loss_gamma
        self.matrix_gamma = matrix_gamma
        self.patch_factor = patch_factor
        self.ave_spectrum = ave_spectrum
