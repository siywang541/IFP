...
from others.frequency_operate import magnitude_phase_split, magnitude_phase_combine
try:
...
def parse_option():
    parser = argparse.ArgumentParser('IFP pre-training script', add_help=False)
    ...
    parser.add_argument('--data-path', type=str, help='path to dataset',default='./dataset/')
    ...
    parser.add_argument('--output', default='output', type=str, metavar='PATH',
                        help='root of output folder, the full path is <output>/<model_name>/<tag> (default: output)',default='./results/')
...
def load_model_pth(network,load_path):
...
def train_one_epoch(config, model, data_loader, optimizer, epoch, lr_scheduler, log_writer):
    ...
    for idx, (img, img_lq, mask, _) in enumerate(data_loader):
        ...
        loss,_,_,_ = model(img, img_lq, mask)
